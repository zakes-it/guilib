from nibbler import *
import argparse
import os
import sys
import json
from Foundation import NSCalendar, NSDate, NSDateComponents, \
    NSGregorianCalendar, NSCalendarUnitYear, NSCalendarUnitMonth, \
    NSCalendarUnitDay

def get_args():
    parser = argparse.ArgumentParser(description=('Displays a window with the '
        'input list as a popup selection and writes the selected item as a '
        'string to stdout on submission'))
    parser.add_argument('--title', help='Window title')
    parser.add_argument('--prompt', help='Window informative text')
    parser.add_argument('--default', help='Default date in YYYY-MM-DD format')
    parser.add_argument('--button', help='Submission button title')
    args = parser.parse_args()
    return args


def submit_button():
    try:
        selection = str(n.views['date'].dateValue())
        date = selection.split()[0]
        sys.stdout.write(date)
        sys.stdout.flush()
    except:
        pass
    sys.stdout.flush()
    n.quit()


def str_to_NSDate(datestr):
    cal = NSCalendar.alloc().initWithCalendarIdentifier_(NSGregorianCalendar)
    datecomp = NSDateComponents.alloc().init()
    yr, mo, day = [int(i) for i in datestr.split('-')]
    datecomp.setYear_(yr)
    datecomp.setMonth_(mo)
    datecomp.setDay_(day)
    return cal.dateFromComponents_(datecomp)


def init_window():
    n.win.becomeMainWindow()
    n.win.center()
    n.win.setCanBecomeVisibleWithoutLogin_(True)
    n.win.orderFrontRegardless()


def set_window(args):
    n.attach(submit_button, 'button')
    if args.title:
        n.win.setTitle_(args.title)
    if args.prompt:
        n.views['label'].setString_(args.prompt)
    if args.default:
        date = str_to_NSDate(args.default)
    else:
        date = NSDate.now()
    n.views['date'].setDateValue_(date)
    if args.button:
        n.views['button'].setTitle_(args.button)


def main():
    args = get_args()
    init_window()
    set_window(args)
    n.run()


if __name__ == '__main__':
    PATH = os.path.abspath(os.path.dirname(__file__))
    try:
        # Because of how this loads, this should be an absolute path if you don't
        # want to run this from the same directory as the script
        nib = os.path.join(PATH, 'nibs', 'Date.nib')
        n = Nibbler(nib)
    except IOError:
        print("Unable to load nib!")
        exit(20)
    main()
